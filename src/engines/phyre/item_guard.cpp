...
//
// The Totori equipment scan guard and the load-time save repair. What the
// defect is, how the damage arises and what the guard does about it are in
// item_guard.h. What is here is the per-build wiring, the decoded table bounds
// and the notes that only mean anything beside the code they sit on.
...
int STDMETHODCALLTYPE traitScanDetour(uintptr_t item, int32_t type) {
  ...
  for (size_t i = 0; i < kTraitCount; ++i) {
    ...
    // totori-ml 0x477d97).
    //
    // Reading a third field would make the scan accept a record the engine
    // rejects. Not reachable from a healthy save, but the whole value of this
...
SanitizeSummary sanitizeLoadedEquipment() {
  ...
  // It is the safest fallback for a damaged value when at least three of the
  // seven core sets carry it; otherwise retain the constructor value zero.
