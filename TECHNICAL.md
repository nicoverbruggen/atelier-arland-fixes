...
The composite is repeated once more immediately after the real Present, into the buffer the present rotated in. Nothing on screen depends on that repeat — the next frame overwrites the same buffer before it is ever shown — and it exists for readers rather than for the display. Without the redirect these games render straight into the backbuffer, so from the moment the frame is drawn until the next one starts, the backbuffer holds the finished picture; under the redirect it holds the *previously presented* frame for all of that time, because the only write to it happens inside the Present hook. Anything sampling the backbuffer at another moment therefore reads a stale frame, and the Steam overlay's screenshot capture is exactly such a reader: it grabs the backbuffer from its own Present hook, and when that hook sits outside the mod's it runs before the downscale, saving the last presented image — which already had Steam's own frame-rate counter and notification pop-ups composited into it. Repeating the pass after the present restores the invariant the redirect broke, at one fullscreen pass per frame; `ARLAND_BACKBUFFER_REFRESH=0` disables it for comparison. Which side of the mod's hook another overlay ended up on is reported once, a few hundred frames in, under verbose logging.
