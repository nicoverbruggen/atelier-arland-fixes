...
- The high-resolution UI text feature rasterizes glyphs with [stb_truetype](https://github.com/nothings/stb) (Sean Barrett, public domain), vendored unchanged under `vendor/stb`. Its bundled replacement typefaces are chosen per game in code rather than by a setting: National Park SemiBold (Rorona) and Nunito Regular (Totori), both under the SIL Open Font License, and Cosmetica Medium (Meruru), an emboldened MgOpen Cosmetica under the MgOpen licence and renamed as that licence requires. A fourth face, Arland Fallback, supplies the symbols the other three do not carry; its outlines are taken from Inter and Source Sans 3, both under the SIL Open Font License, and it is renamed as both licences require. All are embedded in the DLL (generated at build time from the vendored `.ttf` files under `vendor/font/` by `scripts/embed_font.py`, so no large byte arrays are checked in); a `arland-hires-font.ttf` placed beside the DLL overrides them. `[Rendering] Font` selects the mode, not the face.
...
The game's English text also carries characters that look like ASCII but are not, and a Latin font has no glyph for them: full-width digits and letters (`１２３`, U+FF01–FF5E), CJK label brackets (`【】`), Unicode Roman numerals (`Ⅰ–Ⅻ`, used for dungeon tiers), the katakana middle dot (`・`, which starts each line of a bullet list), and the wave dash and hyphen variants. The decoder folds each to its ASCII or Latin equivalent before layout, which keeps the character in the string's own typeface.

What is left after that is real symbols, and those come from a second bundled face rather than from a fold. Eight characters in the three games' English text are missing from the face that game uses, and every one is a symbol: `⇔` in Totori's travel routes (129 strings), and `♪ ※ △ ○ →` in Meruru's tips plus the `α β γ` its quest names are tiered with. A glyph the game's own face lacks is rasterized from Arland Fallback, built by `scripts/subset_fallback_font.py`, at that face's own scale for the same line height so it matches its neighbours in size. Arland Fallback takes its outlines from two typefaces because no one face draws all of these well at text size. Inter supplies most of them: its triangles and circles match the cap height of the words around them, its arrows carry the weight, and it is the only source with the travel-route arrow and the reference mark. Source Sans 3 supplies the musical notes, where Inter's note head is too small for its stem, and the Greek letters. A dozen faces were compared against the real lines; the ones with the widest symbol coverage (DejaVu Sans, Symbola, STIX) all draw a thin note that reads as a different font dropped into the sentence, and the music faces (Noto Music, Bravura) draw theirs for a notation stave, where it hangs below a line of text. The subset holds those eight plus the rest of each family they come from, which is what stops the next string reaching for a sibling it does not have. It holds no letters, because the two faces do not match in design: a symbol standing on its own does not read as a style break, and a letter in the middle of a word would. Kerning is looked up in the primary face only, which returns nothing across a fallback glyph, because the two faces have no kerning relationship.

The face is 68 glyphs and 8 KB, most of the saving coming from what was thrown away rather than what was kept. stb_truetype reads outlines and a character map and nothing else, so the hinting programs, the glyph names and the OpenType layout tables go, as do the name records in every language the source ships, which carry the license text the release already ships as a file. Those tables alone were 40 KB.

Only when neither face has the character does the whole string fall back to the `upscaled` path. In practice that means the controller-button icons and the Japanese labels left behind in the English builds, so no glyph is ever dropped or shown as a tofu box. Before the fallback existed, one such character sent the whole line down that path: 154 of Totori's strings and 41 of Meruru's rendered as the engine's baked art because of a single symbol.

The replacement typefaces and the fallback face are compiled into the DLL from the vendored `.ttf` files under `vendor/font/` (the byte arrays are generated at build time by `scripts/embed_font.py`, not checked in), so the feature needs no loose font file; a `arland-hires-font.ttf` beside the DLL overrides the per-game face, and the fallback applies to it as well. The override is looked for by swapping the DLL's own file name for the font's, which the mod declines when the result would not fit `MAX_PATH`, so an install path deep enough to overflow that buffer keeps the embedded font instead. The substitution is wired only for the English executables, because the per-game text allocator and consumer hooks resolve English-build addresses. On the multilingual builds those hooks stay unresolved and every mode is a safe no-op, so Japanese and Chinese text is never touched (and neither Latin font carries CJK glyphs regardless).
