...
- The high-resolution UI text feature rasterizes glyphs with stb_truetype (Sean Barrett, public domain), vendored unchanged under `vendor/stb`. Its bundled replacement typeface is Cuprum (SIL Open Font License), embedded in the DLL as `src/embedded_font.cpp`; a `arland-hires-font.ttf` placed beside the DLL overrides it.
...
`BattleCutInDimming` governs brightness. It defaults to `false`, which holds the sixteen-byte scene-light parameter at `1.0` and keeps the floor lit; setting it `true` restores the vanilla darkening. `BattleCutInShadows` governs reception. It defaults to `true`, forcing the receiver material's `diffuse` back to `1.0` immediately before each shadow-receiving ground draw through a bounded sixteen-byte update over the `[832, 848)` field, which reopens the reception gate. The two are independent, so the vanilla darker cut-in can be kept while shadows are still restored, or either half can be disabled. The `ARLAND_CUTIN_SHADOWS` and `ARLAND_CUTIN_DIMMING` environment variables override the respective keys for a session, and the per-game defaults live in the capability matrix in `src/game.cpp`.
...
`replaced` re-renders each string from a bundled scalable font (Cuprum), rasterized with stb_truetype through a glyph-atlas cache so each glyph is drawn once and then reused. Matching the engine's layout took reverse-engineering the text renderer, and three points matter:
...
The replacement typeface is compiled into the DLL (`src/embedded_font.cpp`), so the feature needs no loose font file; a `arland-hires-font.ttf` beside the DLL overrides it. The substitution is wired only for the English executables, because the per-game text allocator and consumer hooks resolve English-build addresses. On the multilingual builds those hooks stay unresolved and every mode is a safe no-op, so Japanese and Chinese text is never touched (and Cuprum carries no CJK glyphs regardless).
