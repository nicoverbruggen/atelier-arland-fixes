...
- The high-resolution UI text feature rasterizes glyphs with stb_truetype (Sean Barrett, public domain), vendored unchanged under `vendor/stb`. Its bundled replacement typefaces are National Park (the default) and Cuprum, both under the SIL Open Font License, embedded in the DLL as `src/embedded_font_np.cpp` and `src/embedded_font.cpp` and selected by `[Rendering] FontName`; a `arland-hires-font.ttf` placed beside the DLL overrides them.
